...
export class DashboardVaultService {
	...
	getHeatmapData(): HeatmapData {
		...
		for (const file of this.getManagedMarkdownFiles()) {
	...
	private collectLinkState(files: TFile[]): VaultLinkState {
		...
		for (const [source, targets] of Object.entries(resolved)) {
			if (!markdownPaths.has(source)) continue;
	...
	private getManagedMarkdownFiles(): TFile[] {
		const raw = this.getSettings().xboardDataFolder.trim();
		if (!raw) return this.app.vault.getMarkdownFiles();
		const dataFolder = normalizePath(raw);
		return this.app.vault.getMarkdownFiles().filter(
			(file) => file.path !== dataFolder && !file.path.startsWith(`${dataFolder}/`),
		);
	}
