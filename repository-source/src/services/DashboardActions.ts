...
import type { QuickNoteDefinition, QuickNoteValues } from '../quick-notes/types';
import type {
	DashboardTask,
	DashboardTaskInput,
	DashboardTaskMove,
...
export class DashboardActions {
	...
	async createQuickNote(definition: QuickNoteDefinition, values: QuickNoteValues): Promise<TFile> {
		const file = await this.quickNoteService.createNote(definition, values);
		if (definition.openAfterCreate) await this.app.workspace.getLeaf('tab').openFile(file);
		return file;
	}
